...
//mongoose.connect('mongodb://120.124.134.205:27017/LILY');
...
var itemSchema = new mongoose.Schema({ //商品資訊
	id:String,
	name:String,
	price:Number,
	type:String,
...
var adSchema = new mongoose.Schema({ //商品資訊
	id : String,
	name:String,
	price:Number,
	local_x:Number,
...
var adModel = db.model('ad',adSchema); 
//-----------------------------^資料集綱要、模型宣告
	...
	usersModel.findOne({account:account},function(err,docs){ //取得登入帳號資訊	
		if(docs!=null){ //判斷是否有找到東西
			 if(docs.account!=account || docs.password!=password){ //檢查帳號密碼是否錯誤
				...
				res.end('Account or Password ERROR');
			}else{
				if(account=='admin'){
					console.log('admin!');
					res.cookie('isLogin','admin',{ maxAge: 60000});
					res.end("admin");
					...
					res.cookie('isLogin','user',{ maxAge: 60000});
					res.end("user");
					/*itemModel.find({},function(err,docs){// 搜尋是否重複註冊
						docs=JSON.stringify(docs);
						if(err){
...
app.post('/additem',upload.single('pic'), function(req, res){ 
	...
	itemModel.find({id:id},function(err,docs){// 搜尋是否重複註冊
		...
		{	
			...
			console.log('商品ID重複');
			res.end('ID is repeat');
		}
  });
});
app.post('/addad',upload.single('pic'), function(req, res){
	var id = req.body.id;
	var name = req.body.name;
	var price = req.body.price;
	var local_x = req.body.local_x;
	...
	adModel.find({id:id},function(err,docs){// 搜尋是否重複註冊
		docs=JSON.stringify(docs);
		if(err){
			//console.log(err);
			console.log('存取錯誤，請重試。');
...
app.post('/editad',upload.single('pic'), function(req, res){ 
	var id = req.body.id;
	var name = req.body.name;
	var price = req.body.price;
	var type = req.body.type;
...
app.get('/addad',function(req,res){  
	res.sendFile(__dirname + '/www/addad.html');
});
...
app.get('/admin',function(req,res){  
		itemModel.find({},function(err,docs){// 搜尋是否重複註冊
			...
			if(err){
				...
				data = JSON.parse(docs);
				adModel.find({},function(err,docs){// 搜尋是否重複註冊
					docs=JSON.stringify(docs);
					if(err){
						console.log('存取錯誤，請重試。');
						res.end('存取錯誤，請重試。');
...
app.get('/removead', function (req, res) {
	var id = req.query.id;
	adModel.remove({id:id},function(err){
		console.log('刪除完畢');
		res.redirect('/');
		res.end('刪除完畢');
		});
...
app.get('/editad', function (req, res) {
	var id = req.query.id;
	console.log(id);
	itemModel.findOne({id:id},function(err,docs){// 搜尋是否重複註冊
		docs=JSON.stringify(docs);
